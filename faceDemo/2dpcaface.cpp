...
void PCA2DFaces::train(InputArrayOfArrays _src, InputArray _local_labels)
{
    ...
    vector<Mat> labels = _local_labels.getMat(); // vector<int> labels
    vector<Mat> images = _src.getMat(); // vector<Mat> images
    ...
    if((_num_components <= 0) || (_num_components > n))
        _num_components = n;
//    Mat::zeros()
    Mat DataSum = Mat::zeros(m, n, CV_8UC1);  //add all the samples together
    Mat DataMean = Mat::zeros(m, n, CV_8UC1); // compute the mean of samples,eigen-face
    ...
    Mat Covariance= Mat::zeros(m, n, CV_8UC1);  // get the covariance matrix
    Mat DataTemp = Mat::zeros(m, n, CV_8UC1); //matrix temp to get Gt = E[(A – EA)T (A – EA)]
    for (unsigned int i = 0; i < numSamples; ++i)
    {
    ...
    for(unsigned int sampleIdx = 0; sampleIdx < numSamples; sampleIdx++)
    {
        ...
        Mat ProjectMatrix = Mat::zeros(n, dimSpace, CV_8UC1);
        ...
        // X = ProjectData;
        Mat ProjectData = Mat::zeros(m, dimSpace, CV_8UC1);
        // Y = ProjectData;
...
Ptr<FaceRecognizer> PCA2DFaceRecognizer(int num_components)
{
